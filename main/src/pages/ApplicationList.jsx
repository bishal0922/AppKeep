...
const ApplicationList = ({applications, setApplications}) => {
    ...
    const handleDeleteButton = (rowIndex, companyName) => {
        //ask to make sure
        if (window.confirm(`Are you sure you want to delete ${companyName}?`)) {
          const newApplications = [...applications];
          newApplications.splice(rowIndex, 1);
          setApplications(newApplications);
        }
    }
    ...
    const renderRow  = (row, rowIndex) => {
        ...
        return (
          <tr key={rowIndex} className="table-body-row">
            ...
            <td className="td-last-updated-date-and-delete">
            
            {row?.date}

            <button
                className="td-delete-button"
                onClick={() => handleDeleteButton(rowIndex, row?.companyName)}
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="16"
